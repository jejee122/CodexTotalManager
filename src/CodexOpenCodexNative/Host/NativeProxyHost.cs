...
public sealed class NativeProxyHost
{
    ...
    private const int MaximumProviderModels = 10_000;
        ...
        string? dataRootOverride = null)
    {
        ...
        _upstream = upstream ?? new HttpClient(CreateUpstreamHandler())
        {
        ...
        _app.Use(next => async context =>
        {
            ...
            context.Request.Body = new SizeLimitedReadStream(originalBody, MaximumRequestBodyBytes);
            try
            {
                await next(context);
            }
        ...
        if (!string.IsNullOrEmpty(traceRoot) && File.Exists(Path.Combine(traceRoot, "trace-requests.flag")))
        {
            _app.Use(next => async (HttpContext context) =>
            {
                try
                {
                    // Query strings can contain provider-specific data. The
                    // opt-in trace records only framing metadata and must never
                    // be allowed to break inference when its file is unwritable.
                    File.AppendAllText(Path.Combine(traceRoot, "request-trace.log"),
                        $"{DateTime.Now:O} {context.Request.Method} {context.Request.Path} CE={context.Request.Headers.ContentEncoding} CT={context.Request.ContentType} LEN={context.Request.ContentLength}\n");
                }
                catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
                {
                    // Diagnostic trace failure is non-fatal.
                }
                await next(context);
            });
        }
    ...
    public static SocketsHttpHandler CreateUpstreamHandler() => new()
    {
        AutomaticDecompression = System.Net.DecompressionMethods.All,
        PooledConnectionLifetime = TimeSpan.FromMinutes(10),
        EnableMultipleHttp2Connections = true,
        // Provider API keys and official Codex session headers are only valid
        // for the exact configured endpoint. Treat every redirect as an
        // upstream response instead of silently crossing a host boundary.
    ...
    private void MapRoutes(NativeProxyConfig config)
    {
        ...
        {
            if (!Admitted(context, token)) return Results.Json(new { error = new { message = "unauthorized" } }, statusCode: 401);
            using var body = await ReadManagementJsonObjectAsync(context);
        ...
        _app.MapMethods("/api/providers", new[] { "PATCH" }, async (HttpContext context) =>
        {
            ...
            lock (managementGate)
            {
                var currentIndex = config.Providers.FindIndex(provider =>
                ...
                var current = config.Providers[currentIndex];
                var definition = CloneProviderDefinition(current);
                ...
                if (!IsManagerOwnedCliProviderDefinitionAllowed(definition))
                    return Results.Json(new
                    {
                        error = new { message = "manager-owned CLI provider must use the loopback Responses endpoint" }
                    }, statusCode: 400);
                config.Providers[currentIndex] = definition;
                _store.Save(config);
                registry.UpsertProvider(definition);
        ...
        _app.MapPost("/v1/chat/completions", async (HttpContext context, CancellationToken ct) =>
        {
            ...
            if (!result.Streaming)
            {
                ...
                _requestLog.Record(new RequestLogEntry
                {
                    ...
                    Status = upstream is null
                        ? "error"
                        : IsIncompleteFinishReason(upstream.Choices.FirstOrDefault()?.FinishReason)
                            ? "incomplete"
                            : "completed",
            ...
            await foreach (var adapterEvent in result.Events!)
            {
                ...
                if (adapterEvent.Type is "finish" or "done"
                    && IsIncompleteFinishReason(adapterEvent.FinishReason))
                    status = "incomplete";
                if (adapterEvent.Type is "error" or "incomplete")
                {
                    var errorType = adapterEvent.Type == "error"
                        ? adapterEvent.ErrorType ?? "upstream_error"
                        : "upstream_incomplete";
                    var errorMessage = adapterEvent.Text
                                       ?? (adapterEvent.Type == "error" ? "上游返回错误" : "上游流未完整结束");
                    var errorLine = JsonSerializer.Serialize(new
                    {
                        error = new { message = errorMessage, type = errorType }
                    });
                    await context.Response.WriteAsync($"data: {errorLine}\n\n", ct);
                    break;
                }
        ...
        _app.MapPost("/v1/responses", async (HttpContext context, CancellationToken ct) =>
        {
            ...
            _requestLog.Record(new RequestLogEntry
            {
                ...
                HttpStatus = bridgeStream.Status is "completed" or "incomplete" ? 200 : 500,
                ElapsedMs = sw.ElapsedMilliseconds,
        ...
        _app.MapPost("/v1/messages", async (HttpContext context, CancellationToken ct) =>
        {
            ...
            if (!result.Streaming)
            {
                ...
                _requestLog.Record(new RequestLogEntry
                {
                    ...
                    Model = route.ModelId,
                    Provider = route.ProviderId,
                    Status = IsIncompleteFinishReason(result.FinishReason) ? "incomplete" : "completed",
                    HttpStatus = result.StatusCode,
                    ElapsedMs = sw.ElapsedMilliseconds,
            ...
            _requestLog.Record(new RequestLogEntry
            {
                ...
                HttpStatus = outbound.Status is "completed" or "incomplete" ? 200 : 500,
                ElapsedMs = sw.ElapsedMilliseconds,
        ...
        out string error)
    {
        ...
        error = string.Empty;
        var id = ReadOptionalString(root, "name");
        if (string.IsNullOrWhiteSpace(id)
            ...
            || id.Equals("cmm", StringComparison.OrdinalIgnoreCase))
        {
        ...
        var adapter = ReadOptionalString(providerNode, "adapter");
        var baseUrl = ReadOptionalString(providerNode, "baseUrl");
        var apiKey = ReadOptionalString(providerNode, "apiKey");
        ...
        if (id.StartsWith("cmm-", StringComparison.OrdinalIgnoreCase)
            && !IsManagerOwnedCliProviderDefinitionAllowed(id, adapter, baseUrl, allowPrivate))
        {
            error = "manager-owned CLI provider must use the loopback Responses endpoint";
            return false;
        }
        ...
        var modelIds = modelsNode.ValueKind == JsonValueKind.Array
            ? modelsNode.EnumerateArray()
                ...
                .Cast<string>()
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .Take(MaximumProviderModels + 1)
                .ToList()
            : new List<string>();
        if (modelIds.Count is 0 or > MaximumProviderModels)
        {
            error = modelIds.Count == 0
                ? "provider model list is empty"
                : $"provider model list exceeds {MaximumProviderModels}";
            return false;
        }
        ...
        out string error)
    {
        ...
        var alias = ReadOptionalString(comboNode, "alias");
        ...
        foreach (var targetNode in targetsNode.EnumerateArray())
        {
            ...
            {
                error = "combo target must be an object";
                return false;
            }
            var providerId = ReadOptionalString(targetNode, "provider");
            var model = ReadOptionalString(targetNode, "model");
        ...
        // 诚实失败关闭：路由当前只会使用第一个目标（RouteResolver 取 FirstOrDefault），
        // 多目标组合会被静默降级成单目标，产生"已配置兜底"的错觉；因此这里直接拒绝，
        // 等真正的多目标轮换实现后再放开。
        if (targets.Count > 1)
        {
            error = "multi-target combos are not implemented yet; routing only uses the first target, so only single-target combos are accepted";
            return false;
        }
        combo = new ComboDefinition
        {
    ...
    private static bool IsManagerOwnedCliProviderDefinitionAllowed(ProviderDefinition definition) =>
        IsManagerOwnedCliProviderDefinitionAllowed(
            definition.Id,
            definition.Adapter,
            definition.BaseUrl,
            definition.AllowPrivateNetwork);

    private static bool IsManagerOwnedCliProviderDefinitionAllowed(
        string id,
        string adapter,
        string baseUrl,
        bool allowPrivateNetwork)
    {
        if (!id.StartsWith("cmm-", StringComparison.OrdinalIgnoreCase)) return true;
        return allowPrivateNetwork
               && adapter.Equals("openai-responses", StringComparison.OrdinalIgnoreCase)
               && Uri.TryCreate(baseUrl, UriKind.Absolute, out var uri)
               && uri.Scheme.Equals(Uri.UriSchemeHttp, StringComparison.OrdinalIgnoreCase)
               && uri.Host.Equals("127.0.0.1", StringComparison.Ordinal)
               && uri.AbsolutePath.TrimEnd('/').Equals("/v1", StringComparison.Ordinal);
    }

    private static ProviderDefinition CloneProviderDefinition(ProviderDefinition source) => new()
    {
        Id = source.Id,
        Name = source.Name,
        ...
        Extra = source.Extra is null
            ? null
            : new Dictionary<string, JsonElement>(source.Extra, StringComparer.OrdinalIgnoreCase)
    };
    ...
    private static async Task<JsonDocument?> ReadManagementJsonObjectAsync(HttpContext context)
    {
        JsonDocument body;
        try
        {
        ...
        catch (JsonException)
        {
            ...
            await context.Response.WriteAsJsonAsync(new
            {
                error = new { message = "请求体不是合法 JSON" }
            }, cancellationToken: context.RequestAborted);
            return null;
        }

        if (body.RootElement.ValueKind == JsonValueKind.Object) return body;
        body.Dispose();
        context.Response.StatusCode = StatusCodes.Status400BadRequest;
        await context.Response.WriteAsJsonAsync(new
        {
            error = new { message = "请求体必须是 JSON 对象" }
        }, cancellationToken: context.RequestAborted);
        return null;
    }
    ...
    private static async Task WritePayloadTooLargeAsync(HttpContext context)
    {
        ...
        await context.Response.WriteAsJsonAsync(new
        {
            error = new
            {
                ...
                message = $"请求体不能超过 {MaximumRequestBodyBytes / 1024 / 1024} MB。"
            }
        }, cancellationToken: context.RequestAborted);
    }
        ...
        string? finishReason)
    {
        ...
        var incomplete = IsIncompleteFinishReason(finishReason);
    ...
    private static bool IsIncompleteFinishReason(string? finishReason) =>
        finishReason is "length" or "max_tokens" or "content_filter";
