function Get-DreamSkinCodexConfigPath {
  if ($env:CMM_SANDBOX_CODEX_HOME) { return Join-Path $env:CMM_SANDBOX_CODEX_HOME 'config.toml' }
...
function Get-DreamSkinRuntimeEnginePaths {  param([string]$StateRoot = $(if ($env:CMM_SANDBOX_DREAMSKIN) { $env:CMM_SANDBOX_DREAMSKIN } else { Join-Path $env:LOCALAPPDATA 'CodexDreamSkin' }))
