...
function Get-DreamSkinThemePaths {
  param([string]$StateRoot = $(if ($env:CMM_SANDBOX_DREAMSKIN) { $env:CMM_SANDBOX_DREAMSKIN } else { Join-Path $env:LOCALAPPDATA 'CodexDreamSkin' }))
...
function Initialize-DreamSkinThemeStore {
  param(
    ...
    [string]$StateRoot = $(if ($env:CMM_SANDBOX_DREAMSKIN) { $env:CMM_SANDBOX_DREAMSKIN } else { Join-Path $env:LOCALAPPDATA 'CodexDreamSkin' })
  )
...
# Live-apply the current active theme to the connected Codex session.
# Symmetric to Invoke-DreamSkinLiveRemove: uses injector --once to inject CSS
# into the running Codex renderer without restarting Codex.
function Invoke-DreamSkinLiveApply {
  ...
  if ($null -eq $session) {
    return [pscustomobject]@{
      Attempted = $false
      Applied = $false
      Message = '没有可连接的活动会话；如需应用主题请先启动 Codex。'
    }
  }
  ...
  if ($apply.ExitCode -eq 0) {
    if ($token) {
      $null = Show-DreamSkinOperationUi -Session $session -Phase finish -Token $token `
        -UiState success -Message '皮肤已应用' -TimeoutMs 1500
    }
    return [pscustomobject]@{
      Attempted = $true
      Applied = $true
  ...
  if ($token) {
    $null = Show-DreamSkinOperationUi -Session $session -Phase finish -Token $token `
      -UiState error -Message '应用失败，请重试' -TimeoutMs 1500
  }
  return [pscustomobject]@{
    Attempted = $true
    Applied = $false
    Message = '实时应用失败；可确认重启 Codex 后应用。'
  }
}
