...
$verifyExitCode = 1
try {
$StatePath = (Join-Path $(if ($env:CMM_SANDBOX_DREAMSKIN) { $env:CMM_SANDBOX_DREAMSKIN } else { Join-Path $env:LOCALAPPDATA 'CodexDreamSkin' }) 'state.json')
...
$themePaths = Get-DreamSkinThemePaths -StateRoot $(if ($env:CMM_SANDBOX_DREAMSKIN) { $env:CMM_SANDBOX_DREAMSKIN } else { Join-Path $env:LOCALAPPDATA 'CodexDreamSkin' })
