...
public sealed class AccountUsageLedgerService : IDisposable
{
    ...
    private string? _dailySeriesFingerprint;
    private IReadOnlyList<DailyTokenUsagePoint>? _dailySeriesCache;

    /// <summary>
    /// 最近 N 天（本地时区自然日）的 token 用量聚合，用于用量曲线展示。
    /// 只读旁路：按同样的文件锁规矩流式扫原始尝试日志，按 (段文件名,长度,最后写入) 指纹缓存，未变化不重算。
    /// </summary>
    public async Task<IReadOnlyList<DailyTokenUsagePoint>> ReadDailyTokenSeriesAsync(
        int days = 14,
        CancellationToken cancellationToken = default)
    {
        if (days is < 1 or > 120) throw new ArgumentOutOfRangeException(nameof(days));
        await _gate.WaitAsync(cancellationToken).ConfigureAwait(false);
        try
        {
            ...
            var fingerprint = string.Join(
                ...
                segments.Select(path =>
                    path + ":" + new FileInfo(path).Length.ToString(CultureInfo.InvariantCulture)
                    + ":" + File.GetLastWriteTimeUtc(path).Ticks.ToString(CultureInfo.InvariantCulture)));
            if (_dailySeriesCache is not null
                && string.Equals(_dailySeriesFingerprint, fingerprint, StringComparison.Ordinal))
            {
                return _dailySeriesCache;
            }
            ...
            await using (await AcquireFileLockAsync(AttemptLockPath, cancellationToken).ConfigureAwait(false))
            {
                foreach (var path in segments)
                {
                    ...
                    while (await reader.ReadLineAsync(cancellationToken).ConfigureAwait(false) is { } line)
                    {
                        ...
                        AccountUsageAttemptFact? fact = null;
                        try { fact = JsonSerializer.Deserialize<AccountUsageAttemptFact>(line, DailySeriesJsonOptions); }
                        catch (JsonException) { continue; }
                        if (fact is not null) facts.Add(fact);
                    }
                }
            }
            var series = AggregateDaily(facts, days, UtcNow());
            _dailySeriesFingerprint = fingerprint;
            _dailySeriesCache = series;
            return series;
        }
        finally { _gate.Release(); }
    }

    private static readonly JsonSerializerOptions DailySeriesJsonOptions = new()
    {
        PropertyNameCaseInsensitive = true
    };

    /// <summary>
    /// 纯聚合：只统计成功且被选中的尝试，按幂等键去重，按本地自然日分桶；
    /// 返回从最早覆盖日到今天（含今天）的连续序列，缺数据的天补零。可独立单元测试。
    /// </summary>
    internal static IReadOnlyList<DailyTokenUsagePoint> AggregateDaily(
        IEnumerable<AccountUsageAttemptFact> facts,
        int days,
        DateTimeOffset now)
    {
        var todayLocal = now.ToLocalTime().Date;
        var firstDay = todayLocal.AddDays(-(days - 1));
        var seen = new HashSet<string>(StringComparer.Ordinal);
        var buckets = new Dictionary<DateOnly, (long In, long Out, long Total, int Count)>();
        foreach (var fact in facts)
        {
            if (!fact.Selected || fact.Result != RuntimeExecutionOutcome.Succeeded) continue;
            if (!seen.Add(fact.IdempotencyKey)) continue;
            var usage = fact.Usage;
            if (usage is null) continue;
            var day = DateOnly.FromDateTime((fact.OccurredAt ?? fact.RecordedAt).ToLocalTime().Date);
            if (day < DateOnly.FromDateTime(firstDay)) continue;
            var input = usage.InputTokens ?? 0;
            var output = usage.OutputTokens ?? 0;
            var total = usage.TotalTokens ?? input + output;
            var existing = buckets.TryGetValue(day, out var bucket) ? bucket : (0, 0, 0, 0);
            buckets[day] = (existing.Item1 + input, existing.Item2 + output, existing.Item3 + total, existing.Item4 + 1);
        }
        var series = new List<DailyTokenUsagePoint>(days);
        for (var offset = days - 1; offset >= 0; offset--)
        {
            var day = DateOnly.FromDateTime(todayLocal.AddDays(-offset));
            series.Add(buckets.TryGetValue(day, out var bucket)
                ? new DailyTokenUsagePoint(day, bucket.Item1, bucket.Item2, bucket.Item3, bucket.Item4)
                : DailyTokenUsagePoint.Empty(day));
        }
        return series;
    }
    ...
    private static string SafeSourceKind(string? source)
    {
        ...
        if (source.StartsWith("opencodex:", StringComparison.OrdinalIgnoreCase)) return "总管家本机引擎用量账本";
