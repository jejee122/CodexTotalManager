...
{
    private const string ServerScriptHash = "0390A2A9B3EAC95D3E74AA34291B5987111775352B265320B34FF59E65853B8A";
    ...
    public IReadOnlyList<string> DiscoveredServerAliases { get; }
    ...
    private string? ServerSshConfig { get; }
    private string? ServerSshConfigPinnedHash { get; }
        ...
        int v2rayProxyPort = 10808)
    {
        ...
        var hasPath = !string.IsNullOrWhiteSpace(serverSshConfigPath);
        var hasHash = !string.IsNullOrWhiteSpace(serverSshConfigSha256);
        if (hasHash && !hasPath)
            throw new ArgumentException("服务器连接配置提供了 SHA-256，但没有提供文件路径。");
        if (!hasPath)
        {
        ...
        ServerSshConfig = Path.GetFullPath(serverSshConfigPath!);
        if (hasHash)
        {
        ...
        DiscoveredServerAliases = DiscoverMainServerAliases(ServerSshConfig);
    }
    ...
    public async Task<ServerHealthResult> RunServerHealthAsync(CancellationToken cancellationToken = default)
    {
        if (!ServerCheckExists)
            return Failed("没有找到可用的服务器 SSH 配置，或没有发现主服务器。");
        ...
        var aliases = result.Output.Split('\n', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            ...
            .ToArray();
        if (aliases.Length == 0) aliases = DiscoveredServerAliases.ToArray();
        ...
        var countMessage = countMatches
            ? $"动态发现 {servers.Length} 台主服务器"
            : $"本轮发现 {servers.Length} 台，启动时基准是 {ExpectedServerCount} 台";
        var message = success
    ...
    public static IReadOnlyList<string> DiscoverMainServerAliases(string sshConfigPath)
    {
        try
        {
            ...
            foreach (var line in File.ReadLines(sshConfigPath))
            {
                ...
                foreach (var alias in match.Groups[1].Value.Split((char[]?)null, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
                {
                    if (!IsSafeAlias(alias)
                        || alias.EndsWith("-Public", StringComparison.OrdinalIgnoreCase)
                        || alias.EndsWith("-WG", StringComparison.OrdinalIgnoreCase)
                        || !seen.Add(alias))
                        continue;
                    aliases.Add(alias);
                }
