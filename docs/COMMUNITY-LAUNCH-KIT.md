...
6. 创建 `v3.0.0-rc.26` Release，明确写“候选版，不建议生产使用”；
...
`Codex Total Manager 3.0.0-rc.26 — 通用插件、原生模型目录与无重启切换候选版`
...
验证：全解决方案 0 错误/0 警告、30 项安全测试、隔离集成矩阵、10 万条账本压力测试、
假 Chat/Responses 上游两轮连续对话，以及 589 文件候选包安装回路“只验包、不安装”检查均通过。
